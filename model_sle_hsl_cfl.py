...
class embed_net(nn.Module):
    def __init__(self, class_num, no_local='on', gm_pool='on', arch='resnet50', dataset="sysu", plearn=0, stage=23,
                 depth=-1, head=-1, graphw=-1, gatw=-1.0, meanw=1.3, whiten=0, theta1=0.0, lambda1=1.3,edge=256):
        ...
        if self.stage == 23:
            ...
            self.hypergraph = HypergraphConv(theta1=theta1,edges=edge)
        self.soft = nn.Softmax(dim=0)

        self.gat = Graph_Attention_Union(1024, 1024, meanw=self.lambda1)
        if self.whiten == 1:
            self.whiten_o = whitening_scale_shift(1024, 1)
        print(self.whiten)

        self.fi = nn.Sequential(
            nn.Conv2d(1024 * 3, 1024, 1, 1),
            nn.BatchNorm2d(1024),
            nn.ReLU(inplace=True),
        )
        self.init_weights()
    ...
    def forward(self, x1, x2, modal=0):
        ...
        if self.non_local == 'on':
            ...
            if self.whiten == 1:
                ...
                x = x + self.graphw * self.hypergraph(x)
            # CFL
            if self.training:
                x_shape = x.shape[0] // 5
                CNN_RGB = x[0 * x_shape:1 * x_shape]
                CNN_IR = x[2 * x_shape:3 * x_shape]
                ViT_RGB = x[3 * x_shape:4 * x_shape]
                ViT_IR = x[4 * x_shape:5 * x_shape]

                GAT_CNN_RGB_ALL = self.gat(CNN_IR, CNN_RGB) + self.gat(ViT_RGB, CNN_RGB) + self.gat(ViT_IR, CNN_RGB)

                GAT_CNN_IR_ALL = self.gat(CNN_RGB, CNN_IR) + self.gat(ViT_RGB, CNN_IR) + self.gat(ViT_IR, CNN_IR)
                GAT_ViT_RGB_ALL = self.gat(CNN_RGB, ViT_RGB) + self.gat(CNN_IR, ViT_RGB) + self.gat(ViT_IR, ViT_RGB)
                GAT_ViT_IR_ALL = self.gat(CNN_RGB, ViT_IR) + self.gat(CNN_IR, ViT_IR) + self.gat(ViT_RGB, ViT_IR)

                x = torch.cat((x, self.gatw * GAT_CNN_RGB_ALL + CNN_RGB, self.gatw * GAT_CNN_IR_ALL + CNN_IR,
                               self.gatw * GAT_ViT_RGB_ALL + ViT_RGB, self.gatw * GAT_ViT_IR_ALL + ViT_IR), dim=0)
        ...
        if self.training:
            temp = torch.cat((feat, feat_all), dim=1)
            return x_pool, self.classifier(feat), local_feat_list, logits_list, feat_all, temp
        else:
...
if __name__ == '__main__':
    x1 = torch.Tensor(8, 3, 288, 144).cuda()
    x2 = torch.Tensor(4, 3, 288, 144).cuda()
    model = embed_net(class_num=100, no_local='on', gm_pool='on', stage=23, head=4, depth=2).cuda()
    model(x1, x2)
