...
if args.method =='base':
    net = basline_pcb(n_class, no_local='on', gm_pool='on', arch=args.arch, dataset=dataset, plearn=1, stage=23, depth=args.depth, head=args.head)
else:
    net = basline_pcb(n_class, no_local='on', gm_pool='on', arch=args.arch, dataset=dataset, plearn=1, stage=23, depth=args.depth, head=args.head)
net.to(device)    
...
if dataset == 'sysu':
    ...
    model_path =  "./sle_ckpt/sysu/save_model/sysu_adp_joint_co_nog_ch_nog_sq1_aug_G_erase_0.5_p4_n8_lr_0.1_seed_0_localtri_0.0_otri_1_stage_23_depth_2_head_4_pha_1.0_resume_1_49_best.t"
...
if dataset == 'llcm':
    ...
    for trial in range(10):
        ...
        print('Test Trial: {}'.format(trial+1))
        print(
            'FC:     Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
                cmc[0], cmc[4], cmc[9], cmc[19], mAP, mINP))
        print(
