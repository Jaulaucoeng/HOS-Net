...
from model_sle_hsl_cfl import embed_net
from utils import *
from loss import OriTripletLoss,  MRIC
...
parser.add_argument('--graphw', default=1.0, type=float, metavar='graphw', help='use graphw')
parser.add_argument('--gatw', default=1.0, type=float, metavar='graphw', help='use graphw')
parser.add_argument('--theta1', default=0.5, type=float, metavar='theta1', help='use theta1')
parser.add_argument('--mricT', default=3, type=int, metavar='mricT', help='use mricT')
parser.add_argument('--whiten', default=1, type=int,
                    metavar='whiten', help='random seed')
parser.add_argument('--aicT', default=3, type=int,
                    metavar='aicT', help='random seed')
parser.add_argument('--lambda1', default=1.3, type=float,
                    metavar='lambda1', help='random seed')
...
suffix = suffix + '_p{}_n{}_lr_{}_seed_{}_otri_{}_stage_{}_depth_{}_head_{}_resume_{}_graphw_{}_gatw_{}_mricT_{}_whiten_{}_theta_{}_aicT_{}_lambda1_{}'.format(
    args.num_pos, args.batch_size,
    args.lr, args.seed, args.otri,
    args.stage, args.depth, args.head, args.resume,  args.graphw, args.gatw, args.mricT, args.whiten,args.theta1,args.aicT, args.lambda1)
...
net = embed_net(n_class, no_local='on', gm_pool='on', arch=args.arch, dataset=dataset, plearn=args.pl,
                  stage=args.stage, depth=args.depth, head=args.head, graphw=args.graphw, gatw=args.gatw, whiten=args.whiten, theta1=args.theta1,lambda1=args.lambda1)
net.to(device)
cudnn.benchmark = True


if args.dataset == 'sysu':
    model_path = "./sle_hsl_ckpt/sysu/save_model/sysu_adp_joint_co_nog_ch_nog_sq1_aug_G_erase_0.5_p4_n8_lr_0.1_seed_0_localtri_0.0_otri_1_stage_23_depth_2_head_4_pha_1.0_graphw_1.0_theta1_0.5_best.t"
if args.dataset == 'regdb':
    model_path = ''.format(args.trial)
...
criterion_mric.to(device)

if args.optim == 'sgd':
    ignored_params = list(map(id, net.bottleneck.parameters())) \
                     + list(map(id, net.vit.parameters())) \
                     + list(map(id, net.hypergraph.parameters())) \
    ...
    optimizer = optim.SGD([
        ...
        {'params': net.hypergraph.parameters(), 'lr': args.lr},
        {'params': net.gat.parameters(), 'lr': args.lr},
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        featG, outG, feat, out0, zaa, temp_feature = net(input1, input2)
        b_x = featG.shape[0]
        b_x = b_x // 9

        loss_id = criterion_id(out0[0][0:b_x * 3], labels) + criterion_id(out0[0][b_x * 3:b_x * 5],
                                                                          labels_vit) + criterion_id(
            out0[0][b_x * 5:b_x * 7], labels_vit) + criterion_id(out0[0][b_x * 7:b_x * 9], labels_vit)
        for i in range(len(feat) - 1):
            loss_id_temp = criterion_id(out0[i + 1][0:b_x * 3], labels) + criterion_id(out0[i + 1][b_x * 3:b_x * 5],
                                                                                       labels_vit) + criterion_id(
                out0[i + 1][b_x * 5:b_x * 7], labels_vit) + criterion_id(out0[i + 1][b_x * 7:b_x * 9], labels_vit)
            loss_id += loss_id_temp

        loss_id_G = criterion_id(outG[0:b_x * 3], labels) + criterion_id(outG[b_x * 3:b_x * 5],
                                                                         labels_vit) + criterion_id(
            outG[b_x * 5:b_x * 7], labels_vit) + criterion_id(outG[b_x * 7:b_x * 9], labels_vit)
        loss_tri, batch_acc = criterion_tri(featG[0:b_x * 3], labels)

        loss_tri = loss_tri
        loss_tri += criterion_tri(featG[b_x * 3:b_x * 5], labels_vit)[0]
        loss_tri += criterion_tri(featG[b_x * 5:b_x * 7], labels_vit)[0] + \
                    criterion_tri(featG[b_x * 7:b_x * 9], labels_vit)[0]
        ...
        z_x = temp_feature.shape[0] // 9
        VIS_center = 1.0 * (criterion_mric(temp_feature[0 * z_x:1 * z_x], temp_feature[3 * z_x:4 * z_x])[1] +
                            criterion_mric(temp_feature[0 * z_x:1 * z_x], temp_feature[3 * z_x:4 * z_x])[2])

        IR_center = 1.0 * (criterion_mric(temp_feature[2 * z_x:3 * z_x], temp_feature[4 * z_x:5 * z_x])[1] +
                           criterion_mric(temp_feature[2 * z_x:3 * z_x], temp_feature[4 * z_x:5 * z_x])[2])

        M_center = 1.0 * (criterion_mric(temp_feature[5 * z_x:6 * z_x], temp_feature[6 * z_x:7 * z_x])[1] +
                          criterion_mric(temp_feature[5 * z_x:6 * z_x], temp_feature[6 * z_x:7 * z_x])[2] +
                          criterion_mric(temp_feature[7 * z_x:8 * z_x], temp_feature[8 * z_x:9 * z_x])[1] +
                          criterion_mric(temp_feature[7 * z_x:8 * z_x], temp_feature[8 * z_x:9 * z_x])[2])


        if args.aicT == 3:
            loss_mric = criterion_mric(temp_feature[0 * z_x:1 * z_x], temp_feature[2 * z_x:3 * z_x])[0] + \
                        criterion_mric(temp_feature[3 * z_x:4 * z_x], temp_feature[4 * z_x:5 * z_x])[0] + \
                        criterion_mric(temp_feature[5 * z_x:6 * z_x], temp_feature[6 * z_x:7 * z_x])[0] + \
                        criterion_mric(temp_feature[5 * z_x:6 * z_x], temp_feature[7 * z_x:8 * z_x])[0] + \
                        criterion_mric(temp_feature[5 * z_x:6 * z_x], temp_feature[8 * z_x:9 * z_x])[0] + \
                        criterion_mric(temp_feature[6 * z_x:7 * z_x], temp_feature[7 * z_x:8 * z_x])[0] + \
                        criterion_mric(temp_feature[6 * z_x:7 * z_x], temp_feature[8 * z_x:9 * z_x])[0] + \
                        criterion_mric(temp_feature[7 * z_x:8 * z_x], temp_feature[8 * z_x:9 * z_x])[0] + \
                        criterion_mric(VIS_center, IR_center)[0] + \
                        criterion_mric(VIS_center, M_center)[0] + \
                        criterion_mric(IR_center, M_center)[0]
        loss_mric =  loss_mric
        loss = loss_id + loss_id_G + loss_tri + loss_mric
        ...
        mric_loss.update(loss_mric.item(), 2 * input1.size(0))
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'C-Loss: {mric_loss.val:.4f} ({mric_loss.avg:.4f}) '
                  'Accu: {:.2f}'.format(
    ...
    writer.add_scalar('mric_loss', mric_loss.avg, epoch)
...
for epoch in range(100, 120):
    ...
    if epoch >= 0:
        ...
        # save model
        if cmc_att[0] + mAP_att + mINP_att > best_acc:  # not the real best for sysu-mm01
            best_acc = cmc_att[0] + mAP_att + mINP_att
            best_epoch = epoch
            state = {
                'net': net.state_dict(),
                'cmc': cmc_att,
